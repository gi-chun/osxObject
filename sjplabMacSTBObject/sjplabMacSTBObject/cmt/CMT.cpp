...
//    cv::Algorithm::getList(list);
    ...
    nbInitialKeypoints = (int)selected_keypoints.size();
...
std::vector<Cluster> linkage(const std::vector<cv::Point2f>& list)
{
    ...
    {
        int x, y;
        float min = findMinSymetric(dist, used, (int)list.size()+(int)clusters.size(), x, y);
        Cluster cluster;
        cluster.first = x;
        cluster.second = y;
        cluster.dist = min;
        ...
        used[x] = true;
        used[y] = true;
        int limit = (int)list.size()+(int)clusters.size();
...
void CMT::processFrame(cv::Mat im_gray)
{
    ...
    for(unsigned int i = 0; i < keypoints.size(); i++)
    {
        ...
        if(!(isnan(center.x) | isnan(center.y)))
        {
            ...
            {
                for(int i = (int)activeKeypoints.size()-1; i >= 0; i--)
