...
//        _menuTable = [[UITableView alloc]initWithFrame:CGRectMake(SCREEN_WIDTH/4, 0, 0.75*SCREEN_WIDTH,SCREEN_HEIGHT - (SCREEN_HEIGHT - CGRectGetMaxY(self.frame)) )];
        ...
        _menuTable = [[UITableView alloc]initWithFrame:CGRectMake(SCREEN_WIDTH/2, 0,
                                                                  SCREEN_WIDTH/2, SCREEN_HEIGHT - (SCREEN_HEIGHT - CGRectGetMaxY(self.frame)) )];
...
-(void)setHideChildView:(NSInteger)show
{
    if(show){
...
//    _bgView.alpha = 0;
    _bgView.backgroundColor = [UIColor colorWithWhite:0 alpha:0.0]; //alpha:0.5 --> 0.0
    _bgView.alpha = 0;
    ...
    _normalImageView.layer.cornerRadius = 5.0f;
    ...
    _normalImageView.layer.shadowOffset = CGSizeMake(3.0f, 5.0f);
...
-(void)handleTap:(id)sender //Show Menu
{
    ...
    else
    {
        if(!_hideBgView){
            ...
            [_mainWindow addSubview:_buttonView];
    ...
    [delegate didSelectMainView:_kind];
...
//    NSLog(@"selected CEll: %tu",indexPath.row);
    [delegate didSelectMenuOptionAtIndex:_kind:indexPath.row];
